...
export function useNotificationInbox(allEvents: GeoEvent[]) {
  ...
  // Load inbox from database (source of truth) when user is available
  useEffect(() => {
    const init = async () => {
      // If logged in, load from database (source of truth)
      if (user) {
        try {
          const dbInbox = await userState.getUserInbox(user.id);
          saveInbox(dbInbox); // Cache locally
          setInboxEventIds(dbInbox);
          setAppBadge(dbInbox.length);
        } catch (e) {
          console.error("[Inbox] Failed to load from database:", e);
          // Fall back to localStorage cache
          const cached = getStoredInbox();
          setInboxEventIds(cached);
          setAppBadge(cached.length);
        }
      } else {
        // Not logged in - clear inbox
        setInboxEventIds([]);
        setAppBadge(0);
      }
      ...
      if (notifEventId) {
        ...
        // User clicked this notification - remove from inbox
        setInboxEventIds((prev) => {
          ...
          setAppBadge(updated.length);
          if (user) {
            userState.removeFromInbox(user.id, notifEventId);
          }
          return updated;
        });
      }
  ...
  useEffect(() => {
    ...
    if (processedRef.current.size === 0) {
      const stored = getProcessedEvents();

      // FIRST TIME USER: If no processed events stored, mark ALL current events as processed
      // This ensures inbox starts empty - only NEW events (arriving after signup) will appear
      if (stored.size === 0 && allEvents.length > 0) {
        allEvents.forEach((e) => stored.add(e.id));
        saveProcessedEvents(stored);
      }

      processedRef.current = stored;
    }
